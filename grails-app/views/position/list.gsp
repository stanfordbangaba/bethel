			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="title" title="${message(code: 'position.title.label', default: 'Title')}" />
					
						<g:sortableColumn property="description" title="${message(code: 'position.description.label', default: 'Description')}" />
				...
				<g:each in="${positionInstanceList}" status="i" var="positionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${positionInstance.id}">${fieldValue(bean: positionInstance, field: "title")}</g:link></td>
